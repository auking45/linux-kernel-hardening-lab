...
- [x] **Task 3-4:** `CONFIG_GCC_PLUGIN_STACKLEAK` (커널 스택 데이터 소거 및 스택 고갈 공격 방어).
...
- [x] **Task 4-2:** FG-KASLR (Function Granular KASLR - 함수 단위 배치 무작위화).
- [x] **Task 4-3:** `CONFIG_RANDOMIZE_MEMORY` (물리 메모리 다이렉트 매핑 주소 랜덤화).
...
- [x] **Task 5-4:** `CONFIG_PAGE_TABLE_CHECK` (페이지 테이블 오염 및 불법 페이지 매핑 런타임 탐지).
- [x] **Task 5-5:** KPTI (Kernel Page Table Isolation - 유저/커널 페이지 테이블 완전 분리 및 Meltdown 방어).
...
- [ ] **Task 13-1:** ARM64 MTE (Memory Tagging Extension - ARMv8.5/v9 하드웨어 4비트 컬러 태깅, 동기/비동기 트랩, 힙 UAF/OOB 탐지 데모).
- [ ] **Task 13-2:** ARMv9 POE / S1POE (Permission Overlay Extension - ARMv8.9/v9.4 하드웨어 메모리 권한 오버레이 키 실습).
- [ ] **Task 13-3:** AVF (Android Virtualization Framework - pKVM 기반 마이크로 pVM 게스트 격리 및 통신 실습).
